...
public class AggiungiProdottoOrdineController
	...
	private void annulla() {
		dispatcher.renderVista("Ordine", new WrapperInterVista<Utente, GestoreLuogoDiRitrovo, Ordine, Prodotto>(
				this.utente, this.glr, this.ordine, this.prodotto));
	}
	...
	private void aggiungiProdottoAllOrdine(ActionEvent event) {
		...
		this.ordine.setTotaleSpeso(
				this.ordine.getTotaleSpeso() + prodotto.getPrezzo() * Double.parseDouble(this.quantita.getText()));
		dispatcher.renderVista("Ordine", new WrapperInterVista<Utente, GestoreLuogoDiRitrovo, Ordine, Prodotto>(
				this.utente, this.glr, this.ordine, null));
	}
